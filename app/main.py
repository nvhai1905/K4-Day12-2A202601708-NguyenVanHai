...
def readyz(store: ChatStore = Depends(get_store)):
    ...
    503 ở đây khiến LB ngừng gửi traffic, KHÔNG khiến container bị restart.
    """
    if shutdown_guard.draining:
        return JSONResponse(status_code=503, content={"status": "draining"})
    if not store.ping():
...
def chat(
    ...
    """
    # Hai lớp chặn đứng TRƯỚC khi gọi LLM — tiền mất ở bước generate_reply.
    ...
    return {
        "reply": result["text"],
        "client_id": client_id,
        # Số lượt ĐÃ có trước request này — đọc trước khi add_turn nên chưa
        # tính hai lượt vừa ghi.
        "turns_before": len(history),
        "usd_cost": result["usd_cost"],
        "usage": {
            "prompt": result["prompt_tokens"],
