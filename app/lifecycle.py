...
class ShutdownGuard:
    ...
    def start_draining(self, signum=None, frame=None) -> None:
        ...
        """
        self.draining = True
        # Nhường lại cho handler của uvicorn, nếu không server không bao giờ
        # tự tắt và sẽ ăn SIGKILL sau vài chục giây.
        previous = self._previous.get(signum)
        if callable(previous):
            previous(signum, frame)
    ...
    def arm(self) -> None:
        ...
        for sig in (signal.SIGTERM, signal.SIGINT):
            # Nhớ handler cũ TRƯỚC khi ghi đè — mỗi tín hiệu chỉ có 1 handler.
